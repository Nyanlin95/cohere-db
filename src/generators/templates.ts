...
export class TemplateGenerator {
  ...
  private renderClaudeMd(data: TemplateData): string {
    ...
    output += '> AUTO-GENERATED by cohere. Works with Claude, Codex, Antigravity, Xcode.\n\n';
...
`;
  ...
  private generateCheckpointPatterns(schema: UnifiedSchemaInfo): string {
    ...
    return '# Memory & Checkpoint Patterns\n\n' +
      '> AUTO-GENERATED by cohere. Pattern version: 1.0.0\n\n' +
      '## Directory Structure\n\n' +
      '```\n' +
      '.ai/\n' +
      '├── memory/\n' +
...
\`\`\`bash
db-ai handoff --record --status {status} --agentId {agentId}
\`\`\`
...
db-ai handoff --resume <session_id>
...
| Command | Description |
|---------|-------------|
| \`db-ai handoff --record\` | Record current session |
| \`db-ai handoff --list\` | List available sessions |
| \`db-ai handoff --resume <id>\` | Resume from session |
| \`db-ai handoff --status completed\` | Mark session complete |
...
Agent A (in progress)
    ↓
db-ai handoff --record --status paused
    ↓
Agent B receives files
    ↓
db-ai handoff --resume sess_abc123
    ↓
Agent B continues work
    ↓
db-ai handoff --record --status completed
\`\`\`
`;
  }
