...
export async function handoff(options: HandoffOptions): Promise<void> {
  ...
  console.log('Usage:');
  console.log('  db-ai handoff --record           Record current session state');
  console.log('  db-ai handoff --resume <id>     Resume from previous session');
  console.log('  db-ai handoff --list            List available sessions');
  console.log('');
  console.log('Options:');
...
async function listSessions(outputDir: string): Promise<void> {
  ...
  try {
    ...
    if (handoffFiles.length === 0) {
      ...
      console.log('Record a session with: db-ai handoff --record');
      return;
    }
    ...
    console.log('Resume with: db-ai handoff --resume <session_id>');
  } catch (error) {
...
async function resumeSession(outputDir: string, sessionId: string): Promise<void> {
  ...
  if (!handoffFile) {
    ...
    console.log('List sessions with: db-ai handoff --list');
    return;
  }
...
> AUTO-GENERATED by db-ai-context. Read this before continuing.
...
> AUTO-GENERATED by db-ai-context. Active progress snapshot.
